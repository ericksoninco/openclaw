...
describe("stage timing", () => {
  ...
  it("warns only for slow stage summaries by default", () => {
    expect(
      shouldWarnStageSummary({
        totalMs: 9_999,
    ...
    expect(shouldWarnStageSummary({ totalMs: 10_000, stages: [] })).toBe(true);
    expect(
      shouldWarnStageSummary({
        totalMs: 10,
  ...
  it("formats summaries compactly for logs", () => {
    expect(
      formatStageSummary("run stages: runId=r1", {
        totalMs: 80,
        stages: [
    ...
    ).toBe("run stages: runId=r1 totalMs=80 stages=workspace:25ms@25ms,tools:55ms@80ms");
  });
