...
import { type AuthProfileStore, resolveAuthProfileEligibility } from "../../auth-profiles.js";
...
export function prepareEmbeddedRunAuthSelection(params: {
  ...
  const authStore = params.pluginHarnessOwnsTransport
  ...
  const profileOrder = (() => {
    ...
    if (preparedOrder) {
      return preferredProfileId && preparedOrder.includes(preferredProfileId)
        ? [
            preferredProfileId,
            ...preparedOrder.filter((profileId) => profileId !== preferredProfileId),
          ]
        : [...preparedOrder];
    }
    return resolveAuthProfileOrder({
