...
class EmbeddedPiResourceLoader implements ResourceLoader {
  private extensionsResult: ReturnType<ResourceLoader["getExtensions"]> = {
    extensions: [],
    errors: [],
    runtime: createExtensionRuntime(),
  };

  constructor(private readonly params: EmbeddedPiResourceLoaderParams) {}
  ...
  async reload(): Promise<void> {
    ...
    for (const [index, factory] of this.params.extensionFactories.entries()) {
...
export function createEmbeddedPiResourceLoader(
  ...
  return new EmbeddedPiResourceLoader(params);
}
